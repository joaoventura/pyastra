...
def is_above_horizon(ra, decl, mc_ra, lat):
    """
    Returns if an object's 'ra' and 'decl' is above the horizon at a specific latitude,
    given the MC's right ascension.

    This function checks if the equatorial distance from the object to the MC is within its diurnal
    semi-arc.
    
    """
    d_arc, _ = dnarcs(decl, lat)
    dist = abs(angle.closest_distance(mc_ra, ra))
    return dist <= d_arc / 2.0 + 0.0003  # 1 arc-second


# === Coordinate systems === #

def eq_coords(lon, lat):
    """
