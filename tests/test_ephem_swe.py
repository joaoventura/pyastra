...
class SweTests(unittest.TestCase):
    ...
    def test_sun_position(self):
        lon, lat, lon_speed, lat_speed = swe.swe_object(const.SUN, self.date.jd)
    ...
    def test_moon_position(self):
        lon, lat, lon_speed, lat_speed = swe.swe_object(const.MOON, self.date.jd)
    ...
    def test_house1_position(self):
        cusps, _ = swe.swe_houses(self.date.jd, self.pos.lat,
    ...
    def test_asc_position(self):
        cusps, ascmc = swe.swe_houses(self.date.jd, self.pos.lat,
                                      self.pos.lon, const.HOUSES_ALCABITUS)
